...
class CancelledBookings(DashboardModule):
    ...
    def __init__(self, title=None, children=list(), **kwargs):
        ...
        if kwargs.get('context') and kwargs.get('context').get('user'):
            ...
            if current_user.groups.filter(name='Hotel Group Administrators').exists(): 
                ...
                bookings = Booking.objects.filter(booking_status=2)\
                    .values('hotel_group__hotel_group_name', 'hotel_group_id', 'hotel__hotel_name', 'hotel_id', 'hotel__city') \
                    .annotate(total=Count('id')) 
                data = list(bookings)

            kwargs.update({'children': {'data': data, 'prefix': 'cancelled_bookings'}})                  
        
        super(CancelledBookings, self).__init__(title, **kwargs)   