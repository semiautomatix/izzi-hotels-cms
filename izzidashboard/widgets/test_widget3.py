...
class TestWidget3(DashboardModule):
    title = 'Test3'
    template = 'cms/dashboard_modules/bar.html'    
    ...
    def __init__(self, title=None, children=list(), **kwargs):
        ...
        kwargs.update({'children': {'hotel_groups': json.dumps(hotel_groups), 'drilldown_hotels': json.dumps(drilldown_hotels),\
            'drilldown_cities': json.dumps(drilldown_cities), 'prefix': 'test3'}})  

        super(TestWidget3, self).__init__(title, **kwargs)   