        ...
        if kwargs.get('context') and kwargs.get('context').get('user'):
            ...
            if current_user.groups.filter(name='Hotel Group Administrators').exists(): 
                ...
                bookings = Booking.objects.filter(hotel_group_id=hotel_group_id).filter(booking_status=1)\
                    .values('hotel__hotel_name', 'hotel_id', 'hotel__city', 'user__usermetadata__gender') \
                ...
                bookings = Booking.objects.filter(booking_status=1)\
                    .values('hotel_group__hotel_group_name', 'hotel_group_id', 'hotel__hotel_name', 'hotel_id', 'hotel__city', 'user__usermetadata__gender') \
                    .annotate(total=Count('id')) 
                data = list(bookings)

            kwargs.update({'children': {'data': json.dumps(data), 'prefix': 'bookings_by_gender', 'series_label': 'user__usermetadata__gender'}})         
        
        super(BookingsByGender, self).__init__(title, **kwargs)   